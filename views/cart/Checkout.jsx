...
export default function CheckoutView({ shoppingCart }) {
  // Extract products from the shopping cart
  const products = Array.from(shoppingCart.products.values()).map(wrapper => ({
    _id: wrapper.product._id,
    name: wrapper.product.name,
    price: wrapper.product.price,
    quantity: wrapper.count
  }));

  // Calculate total count
  const count = products.reduce((totalCount, product) => totalCount + product.quantity, 0);
              ...
              <ul className="list-group list-group-flush">
                {
                  products.map((wrapper, index) => {
                    console.log(wrapper);
                    return (
                   <li key={wrapper._id} className="list-group-item d-flex justify-content-between lh-sm">
                      <div>
                        <h6 className="my-0">{wrapper.name}</h6>
                        <small className="text-muted">{wrapper.name}</small>
                      </div>
                      <span className="text-muted">${wrapper.price}</span>
                    </li>
                  );
                })
                }
